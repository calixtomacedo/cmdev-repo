...
public class ControllerAdviceExceptionsHandler {
    ...
    public ResponseEntity<MessageErroResponse> handler(RuntimeException exception) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new MessageErroResponse(HttpStatus.INTERNAL_SERVER_ERROR,  exception.getMessage(), builderMessage.toString(), exception.getCause().getMessage()));
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity<List<FieldsErrorResponse>> handler(MethodArgumentNotValidException validException) {
        List<FieldError> fieldErrors = validException.getBindingResult().getFieldErrors();
        List<FieldsErrorResponse> responseErrorsList = new ArrayList<>();
        fieldErrors.forEach(error -> {
            responseErrorsList.add(new FieldsErrorResponse(error.getField(), error.getDefaultMessage()));
        });
